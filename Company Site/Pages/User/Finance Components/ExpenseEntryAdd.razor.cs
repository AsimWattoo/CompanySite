...
{
	public partial class ExpenseEntryAdd : ComponentBase
	{
		...
		/// The new entry
		/// </summary>
		private ExpenseEntry NewEntry { get; set; } = new ExpenseEntry();
		...
		protected override async Task OnInitializedAsync()
		{
			...
			ProtectedBrowserStorageResult<string> res = await _sessionManager.GetAsync<string>("ExpensePageMode");
			if (res.Success)
			{
				if (res.Value == "edit")
					IsAddMode = false;
		...
		{
			await _sessionManager.SetAsync("FinancePageMode", "expense");
